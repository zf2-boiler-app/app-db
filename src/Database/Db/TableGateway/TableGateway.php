<?php
namespace BoilerAppDb\Db\TableGateway;
class TableGateway extends \BoilerAppDb\Db\TableGateway\AbstractTableGateway{
	/**
	 * Constructor
	 * @param string $sTable
	...
	public function __construct($sTable, \Zend\Db\Adapter\Adapter $oAdapter, $sRowGatewayClass){
		...
		$this->featureSet = new \Zend\Db\TableGateway\Feature\FeatureSet(array(
			new \BoilerAppDb\Db\TableGateway\Feature\MetadataFeature(),
			new \BoilerAppDb\Db\TableGateway\Feature\EventFeature(),
			new \BoilerAppDb\Db\TableGateway\Feature\RowGatewayFeature($sRowGatewayClass)
		));
