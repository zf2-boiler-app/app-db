<?php
namespace BoilerAppDb\Db\TableGateway\Feature;
class RowGatewayFeature extends \Zend\Db\TableGateway\Feature\RowGatewayFeature{
	...
	public function postInitialize(){
		if(!($this->tableGateway instanceof \BoilerAppDb\Db\TableGateway\AbstractTableGateway))throw new \Exception(sprintf(
			'This feature "%s" expects the TableGateway to be an instance of \BoilerAppDb\Db\TableGateway\AbstractTableGateway',
			__CLASS__
		));
    	...
    	if($oRowGateway instanceof \BoilerAppDb\Db\RowGateway\AbstractRowGateway)$this->tableGateway->resultSetPrototype->setArrayObjectPrototype($oRowGateway);
    	else throw new \Exception($sRowGatewayClass.' is not an instance of \BoilerAppDb\Db\RowGateway\AbstractRowGateway');
	}
}